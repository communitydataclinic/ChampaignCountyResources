...
@section('content')
                            ...
                            <div class="row">
                                <div class="col-md-4">
                                    <div class="form-group">
                                        ...
                                        <input class="form-control selectpicker" type="text" id="location_name"  name="location_name" value="{{ old('location_name') }}">
                                        @error('location_name')
                                ...
                                <div class="col-md-4">
                                    <div class="form-group">
                                        ...
                                        <select class="form-control selectpicker" data-live-search="true" id="facility_organization"
                                                name="facility_organization" {{old('facility_organization')}} data-size="5">
                                            @foreach($organization_name_list as $key => $org_name)
                                                <option {{ (old("facility_organization") == $org_name ? "selected":"") }} value="{{$org_name}}">{{$org_name}}</option>
                                            @endforeach
                                        </select>
                                    ...
                                    <div class="form-group">
                                        ...
                                        <select class="form-control selectpicker" multiple data-live-search="true" id="facility_service"
                                            ...
                                            @foreach($service_info_list as $key => $service_info)
                                                <option {{ (old("facility_service") && in_array($service_info->service_recordid, old("facility_service")) ? "selected":"") }} value="{{$service_info->service_recordid}}">{{$service_info->service_name}}</option>
                                            @endforeach
                                        </select>
                                    </div>
                                </div>
                                    ...
                                    <div class="form-group">
                                        ...
                                        <select class="form-control selectpicker" multiple data-live-search="true" id="facility_schedules"
                                            ...
                                            @foreach($schedule_info_list as $key => $schedule_info)
                                                <option {{ (old("facility_schedules") && in_array($schedule_info->schedule_recordid, old("facility_schedules")) ? "selected":"") }} value="{{$schedule_info->schedule_recordid}}">{{$schedule_info->schedule_opens_at}} ~ {{$schedule_info->schedule_closes_at}}</option>
                                            @endforeach
                                        </select>
                                    </div>
                                </div-->
                                ...
                                <div class="col-md-6">
                                    <div class="form-group">
                                        ...
                                        <input class="form-control selectpicker" type="text" id="facility_street_address"
                                               name="facility_street_address" value="{{ old('facility_street_address') }}">
                                    </div>
                                </div>
                                <div class="col-md-6">
                                    <div class="form-group">
                                        ...
                                        <select class="form-control selectpicker" data-live-search="true" id="facility_address_city"
                                                name="facility_address_city" {{old('facility_address_city')}} data-size="5">
                                            @foreach($address_city_list as $key => $address_city)
                                                <option {{ (old("facility_address_city") == $address_city ? "selected":"") }} value="{{$address_city}}">{{$address_city}}</option>
                                            @endforeach
                                        </select>
                                    </div>
                                </div>
                                <div class="col-md-4">
                                    <div class="form-group">
                                        ...
                                        <select class="form-control selectpicker" data-live-search="true" id="facility_address_state"
                                                name="facility_address_state" {{old('facility_address_state')}} data-size="5">
                                            @foreach($address_states_list as $key => $address_state)
                                                <option {{ (old("facility_address_state") == $address_state ? "selected":"") }} value="{{$address_state}}">{{$address_state}}</option>
                                            @endforeach
                                        </select>
                                    </div>
                                </div>
    ...
    <script>
        ...
        $(document).ready(function() {
            ...
            @if(! empty(old('facility_organization')))
            $('select#facility_organization').val('{{ old('facility_organization') }}').change();
            @endif

            // $('select#facility_schedules').val([]).change();
            @if(! empty(old('facility_schedules')))
            console.log({!! json_encode(old('facility_schedules')) !!})
            $('select#facility_schedules').val({!! json_encode(old('facility_schedules')) !!} ).change();
            @endif

            //$('select#facility_address_city').val([]).change();
            @if(! empty(old('facility_address_city')))
            $('select#facility_address_city').val('{{ old('facility_address_city') }}').change();
            @endif

            //$('select#facility_address_state').val([]).change();
            @if(! empty(old('facility_address_state')))
            $('select#facility_address_state').val('{{ old('facility_address_state') }}').change();
            @endif
        });
        // $(document).ready(function(){
